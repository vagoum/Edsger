...
and get_indetifier s = match s with
        Eid(a) -> a
        |_ -> ""
and  codegen_expr expr builder= match expr with
        ...
        |Eid a ->let v= try Hashtbl.find named_values a with |Not_found -> raise (Error "uknown variable name") in build_load v a builder
        ...
        |EAmber a -> find_variable (get_indetifier a) (* to see it again*)
        |EPointer a-> let tmp =  find_variable (get_indetifier a) (*to see*) in let load_ = build_load tmp "tmp" builder in let pointerr=build_gep load_ [|(default_val_type TYPE_int)|] "tmp" builder in build_load pointerr "dereference" builder 
        ...
        |EDel a -> build_free (build_load (find_variable (get_indetifier a)) "tmp" builder) builder
...
and  codegen_fuction_call fuction  params builder =
        ...
        build_call (get_function fuction) (Array.of_list actual_params) "tmp" builder
and  codegen_array_access e1 e2 builder = 
        let lvaluee = find_variable (get_indetifier e1)  in 
...
and  codegen_binary e1 e2 expr  builder= 
        ...
        in
            if (get_type e1 = TYPE_int) then int_fun expr else float_fun expr 
    ...
    and codegen_func func =
        ...
        let params = Array.iteri ( fun i el -> let n = List.nth (get_fuction_f func.entry_info).function_paramlist i in let n = n.entry_name in set_value_name n el;Hashtbl.add named_values n el;) (params f) 
        in
  ...
  and codegen_decl decl builder = match decl with 
       ...
       |FunDef (a,b,c) -> make_function FunDef (a,b,c) *)
and findinHash = find_variable 
and getAdreess expr builder =  match expr with 
 Eid(x) ->  findinHash x
 |EAmber (x)->  let y = findinHash (get_indetifier x) in
        let dereference = build_struct_gep y 0 "tmp" builder in build_load dereference "tmp" builder
 |EPointer (x)-> let y = findinHash (get_indetifier x) in let load_ = build_load y "temp" builder in
        let dereference = build_struct_gep load_ 0 "tmp" builder in dereference 
 |EArray(x,y) -> let index = codegen_expr y builder in  let tmp_val = findinHash (get_indetifier x) in
...
let codegen_main main = 
        ...
        let _ = List.map (fun x-> match x with FunDecl(a) -> ignore(codegen_func a )|_->()) main in
